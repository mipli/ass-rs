...
impl Account {
    ...
    pub fn sign_url(&self, url: Url) -> Result<Url, Error> {
        ...
        let s: String = signature
            ...
            .map(|s| format!("{:02x}", s))
            .collect();
        Url::parse_with_params(url.as_str(), &[("accessToken", &s)]).map_err(|err| err.into())
    }

    pub fn from_file<T: Into<PathBuf>>(file: T) -> Result<Self, AssError> {
        let file = file.into();
        let mut file = File::open(&file).map_err(|err| AssError::NotFound {
            err: err.to_string(),
        ...
        serde_json::from_str::<Account>(&contents).map_err(|err| AssError::InvalidAccountFile {
            err: err.to_string(),
            file: err.to_string(),
        })
    }
    ...
    pub fn upload_file<T: Into<PathBuf>>(
        ...
        let file_name = path
            .file_name()
            .ok_or_else(|| AssError::InvalidFile {
